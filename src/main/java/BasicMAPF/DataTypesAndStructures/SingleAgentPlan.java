...
public class SingleAgentPlan implements Iterable<Move> {
    ...
    /**
     * the sum of the time the agent stay in his place during the plan
     */
    private int numOfstay = 0;
    /**
    ...
    /**
     * @return true if the newMove is a liglle move to the agent, false otherwise
     */
    ...
    public void addMove(Move newMove){
        if(isValidNextMoveForAgent(this.moves, newMove, this.agent)){
            ...
            if (this.moves.size() == 1 && newMove.prevLocation.getCoordinate().equals(agent.target)) {
                firstVisitToTargetTime = newMove.timeNow - 1; // started at target  (Reduction to UA)
            }
            else if (newMove.currLocation.getCoordinate().equals(agent.target) && firstVisitToTargetTime == -1){  // if the agent reaches his goal move for the first time -> Update firstVisitToTargetTime
                firstVisitToTargetTime = newMove.timeNow;
            }
            if (newMove.currLocation.getCoordinate().equals(newMove.prevLocation.getCoordinate())){  // if the agent stay in his place for the next move
                numOfstay++;
            }
        }
    ...
    public void clearMoves(){
        this.moves.clear(); // clear the Moves List
        this.firstVisitToTargetTime = -1; // reset the firstVisitToTargetTime
        this.numOfstay = 0; //reset the numOfstay
    }
...
//        }
        ...
        for (Move move: this.moves) {
            if ( !(foundFirstPlace) && move.currLocation.getCoordinate().equals(agent.target)){
                this.firstVisitToTargetTime = move.timeNow;
                foundFirstPlace = true;
            }
            if (move.currLocation.getCoordinate().equals(move.prevLocation.getCoordinate())){  // if the agent stay in his place for the next move
                numOfstay++;
            }
        }
    }
    ...
    /**
     * @return true if the agent has passed in his target (RO), false otherwise
     */
     ...
     */
    public int getCost() {
        // if(size() == 1 && moves.get(0).prevLocation.equals(moves.get(0).currLocation)){  // start point == goal point && List of moves == 1 -> the agent did not move
        //     return 0;
        // }
        // else{
        //     return size();
    ...
    /**
     * @return a string represent the plan for the agent
     * **** right now i add the fuel for check ****
     */
    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        
        sb.append("\nPlan for agent ").append(agent.iD).append(": ");

        if (moves.isEmpty()) {
            sb.append("empty");
        } else {
            sb.append("0:").append(moves.get(0).prevLocation.getCoordinate());
        }
        ...
        for (Move move : this) {
            ...
            .append(move.timeNow)
            .append(':')
            .append(move.currLocation.getCoordinate());
        }
        sb.append(" | Target index:").append(agent.target);
        sb.append(" | Start index:").append(agent.source);
        sb.append(" | Start Time:" ).append(getPlanStartTime());
        sb.append(" | Goal Time: ").append(firstVisitToTargetTime());
        sb.append(" |");
        return sb.toString();
    }
    ...
    /**
     * @return the Service Time of a single agent
     */
    public int serviceTime(){ return firstVisitToTargetTime() - getPlanStartTime();}

    /**
     * @return the fuel (only the times the agent did not stay in his place) of a single agent
     */
    public int fuel(){ return moves.size() - numOfstay;}

    /**
     * @return the number of UA that moved of a single agent
     */
    public int nua(){
        for (Move m : moves){
            if (!m.prevLocation.equals(m.currLocation))
                return 1;
        }
        return 0;
    }
    ...
    public int nua_2(){
        boolean movedFirstTime = false;
        int cost = 0;

        for (Move m : moves){
            //Assigned
            if (firstVisitToTargetTime() >= m.timeNow)
                cost++;
            //Unassigned
            else{
                if (!m.prevLocation.equals(m.currLocation)){
                    if (!movedFirstTime){
                        cost += 1000; //penalty for the first move after target
                        movedFirstTime = true;
                    }
                    else{
                        cost++;
    ...
    public int sst_all(){
        int start = getPlanStartTime();

        int lastMoveTime = start; // Initialize to start time, so if the agent never moves, service time is 0
        for (Move m : moves){
            if (!m.prevLocation.equals(m.currLocation)){
                lastMoveTime = m.timeNow;
            }
        }
        return (lastMoveTime - start);
    }
    ...
    public boolean StayInIndex(int time){
        Move m = moveAt(time);
        if (m == null)
             return true; // before start or after end -> agent is staying
        return m.prevLocation.equals(m.currLocation);
    }
}
