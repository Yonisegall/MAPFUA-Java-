...
public class SingleAgentAStar_Solver extends A_Solver {
    ...
    public SingleAgentAStar_Solver(Comparator<AStarState> stateComparator) {
        ...
        //  this.stateComparator = Objects.requireNonNullElse(stateComparator, new TieBreakingForLowerGAndLessConflicts());
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    }
    /*  = set up =  */
    ...
    @Override
    protected Solution runAlgorithm(MAPF_Instance instance, RunParameters parameters) {return solveAStar();}
    ...
    protected Solution solveAStar() {
        ...
        while ((currentState = openList.poll()) != null){ //dequeu in the while condition
            ...
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // Debug: why is this goal candidate accepted/rejected?
            // System.out.println("LOW-LEVEL POPPED: agent=" + agent.iD
            //     + " goal=" + agent.target
            //     + " time=" + currentState.move.timeNow
            //     + " loc=" + currentState.move.currLocation.getCoordinate()
            //     + " g=" + currentState.g + " h=" + currentState.h + " f=" + currentState.f
            //     + " conflicts=" + currentState.conflicts
            //     + " visitedTarget=" + currentState.visitedTarget
            //     + " prevloc=" + currentState.move.prevLocation.getCoordinate()
            //     + " isMoveFromTarget=" + isMoveFromTarget(currentState.move)
            //     + " isMoveToTarget=" + isMoveToTarget(currentState.move)
            //     + " path=" + currentState.getOrderedMoves());
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            
            {
                ...
                ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // Debug: why is this goal candidate accepted/rejected?
                // int lac = lastRejectionTime;
                // int conflictsIfStay = conflictAvoidanceTable == null ? 0 : conflictAvoidanceTable.numConflicts(currentState.move, true);
                // System.out.println("A* goal-candidate agent=" + agent.iD
                //     + " loc=" + currentState.move.currLocation.getCoordinate()
                //     + " t=" + currentState.move.timeNow
                //     + " lastReject=" + lac
                //     + " isALastMove=" + currentState.isALastMove
                //     + " conflictsIfStay=" + conflictsIfStay
                //     + " g=" + currentState.getG() + " f=" + currentState.getF());



                // // NEW: accept only when no hard rejection AND no CAT conflicts for staying
                // if (lastRejectionTime < currentState.move.timeNow && conflictsIfStay == 0) {
                //     currentState.backTracePlan(this.existingPlan);
                //     System.out.println("Low-level A* returned plan for agent " + agent.iD + ": " + this.existingPlan);
                //     return this.existingSolution;
                // } else {
                //     if (!(this.conflictAvoidanceTable != null && currentState.isALastMove)) {
                //         expand(currentState);
                //     }
                // }
                ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public class AStarState implements Comparable<AStarState>{
        ...
        public AStarState(@NotNull Move move, AStarState prevState, int g, SingleAgentGAndH hFunction, int conflicts, boolean visitedTarget, boolean isALastMove) {
            ...
            this.prev = prevState;
            // if (visitedTarget){
            //     if (prevState != null && !prevState.equals(this))
            //         g = 0;
            // }
            this.g = g;
        ...
        //     // Debug: print candidate moves
        //     System.out.println("backTracePlan: candidate moves for agent " + existingPlan.agent.iD + " -> " + moves);

        //     // Trim to first safe goal visit (if any)
        //     I_Location targetLocation = map.getMapLocation(targetCoor);
        //     int safeGoalIndex = -1;
        //     for (int i = 0; i < moves.size(); i++) {
        //         Move m = moves.get(i);
        //         if (m.currLocation.equals(targetLocation)) {
        //             int lastRejection = constraints == null ? -1 : constraints.lastRejectionTime(m);
        //             int conflictsIfStay = conflictAvoidanceTable == null ? 0 : conflictAvoidanceTable.numConflicts(m, true);
        //             System.out.println("backTracePlan: move t=" + m.timeNow + " loc=" + m.currLocation.getCoordinate()
        //                 + " lastRejection=" + lastRejection + " conflictsIfStay=" + conflictsIfStay);
        //             if (lastRejection < m.timeNow && conflictsIfStay == 0) {
        //                 safeGoalIndex = i;
        //                 System.out.println("backTracePlan: found SAFE goal at index " + safeGoalIndex + " (t=" + m.timeNow + ")");
        //                 break;
        //             }
        //         }
        //     }
        //     if (safeGoalIndex != -1) {
        //         moves = new ArrayList<>(moves.subList(0, safeGoalIndex + 1));
        //         System.out.println("backTracePlan: trimming moves to safe goal -> " + moves);
        //     } else {
        //         System.out.println("backTracePlan: no safe goal found in candidate moves");
        //     }

        //     // Overwrite existingPlan from insertion time onward (avoid appending old suffixes)
        //     if (!moves.isEmpty() && existingPlan.size() > 0) {
        //         int insertionTime = moves.get(0).timeNow;
        //         List<Move> preserved = new ArrayList<>();
        //         int first = existingPlan.getFirstMoveTime();
        //         if (first != -1) {
        //             for (int t = first; t < insertionTime; t++) {
        //                 Move m = existingPlan.moveAt(t);
        //                 if (m != null) preserved.add(m);
        //             }
        //         }
        //         System.out.println("backTracePlan: insertionTime=" + insertionTime + " preservedPrefixSize=" + preserved.size()
        //             + " existingPlanSizeBefore=" + existingPlan.size());
        //         existingPlan.clearMoves();
        //         if (!preserved.isEmpty()) existingPlan.addMoves(preserved);
        //     }

        //     // Append the new moves
        //     existingPlan.addMoves(moves);
        //     System.out.println("backTracePlan: existingPlan after append for agent " + existingPlan.agent.iD + " -> " + existingPlan);
        //     return existingPlan;
        // }
