...
public class ServiceTimeGAndH implements SingleAgentGAndH {
    ...
    public float getH(SingleAgentAStar_Solver.@NotNull AStarState state) {
        ...
        // } else if (state.getPrev() == null && state.visitedTarget) {return 0;}
    ...
    public int cost(Move move, boolean isAfterTargetExcludingFirstMoveToTarget){
        if (isAfterTargetExcludingFirstMoveToTarget) {
            return 0; // constant cost after reaching target (was 0 before)
        }
        return cost(move);
    }
    ...
    // public int cost(Move move, boolean isAfterTargetExcludingFirstMoveToTarget){
    //     if (isAfterTargetExcludingFirstMoveToTarget) {
    //         // Discourage any movement after goal; stays are fine
    //         if (!move.prevLocation.equals(move.currLocation)) {
    //             return Integer.MAX_VALUE / 4; // very high, but finite to allow conflict resolution
    //         }
    //         return 0; // staying after goal is free
    //     }
    //     return cost(move);
    // }
