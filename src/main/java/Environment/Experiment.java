...
public class Experiment {
    ...
    public InstanceReport setReport(MAPF_Instance instance, I_Solver solver) {
        ...
        // Use safe fallback for possibly-null strings
        instanceReport.putStringValue(InstanceReport.StandardFields.experimentName, Objects.toString(this.experimentName, ""));
        instanceReport.putStringValue(InstanceReport.StandardFields.instanceName, Objects.toString(instance.extendedName, instance.name));
        instanceReport.putStringValue(InstanceReport.StandardFields.mapName, Objects.toString(instance.name, ""));
        putMapStats(instanceReport, instance);
        putInstanceStats(instanceReport, instance);
        instanceReport.putStringValue(InstanceReport.StandardFields.solver, Objects.toString(solver == null ? null : solver.getName(), ""));
    ...
    protected boolean runInstanceOnSolver(I_Solver solver, Map<String, Integer> minNumFailedAgentsForInstance, MAPF_Instance instance) {
        ...
        if (solution != null) {
            ...
            if (visualizer != null) {
                try {
                    // Defensive: ensure solution contains a plan object for every agent (prevents visualizer NPE while debugging)
                    for (Agent a : instance.agents) {
                        if (solution.getPlanFor(a) == null) {
                            System.err.println("Warning: solver " + (solver == null ? "null" : solver.getName())
                                    + " returned no plan for agent " + a.iD + " — inserting empty plan for visualization.");
                            solution.putPlan(new SingleAgentPlan(a));
                        }
                    }
                    visualizer.visualizeSolution(instance, solution, solver.getName() + " - " + instanceName);
                }
    ...
    /**
     * @return the SOC of each Individual agent With Priorities
     */
